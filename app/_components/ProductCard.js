...
function ProductCard({ product }) {
  return (
    <div className="bg-white p-[1.2rem] shadow-sm hover:shadow-md transition-all duration-300 cursor-pointer group">
      ...
      {/* Name */}
      <p className="text-[1.4rem] text-left font-medium pb-1.5">
        {product.name}
      </p>

      {/* Brand */}
      <p className="text-[1.2rem] text-left text-gray-500 uppercase tracking-wide">
        {product.brand}
      </p>

      {/* Price */}
      <p className="text-[1.4rem] mt-5 text-left text-brand-blue font-semibold">
        ₦{product.price.toLocaleString()}
      </p>

      {/* Add to Cart */}
      <button className="mt-6 w-full bg-brand-blue px-4 py-3 text-[1.3rem] font-semibold text-white hover:bg-[#1663b1] transition-colors duration-300">
        Add to cart
      </button>
    </div>
  );
